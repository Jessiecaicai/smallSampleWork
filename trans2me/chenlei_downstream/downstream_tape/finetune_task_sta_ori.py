...
if __name__ == '__main__':
    ...
    stability_test_loader = DataLoader(
        stability_valid_data, batch_size=batch_size, shuffle=False, collate_fn=stability_test_data.collate_fn
    )
    ...
    for epoch in range(epochs):
        ...
        print("\nStep: {} / {} finish. Validating Loss: {:.2f}. Validating Spearman’s: {:.2f}.\n".
              format(val_step, len(stability_valid_loader), (val_loss / val_step), (val_p / val_step)))
        val_toc = time.time()
        ...
        if val_p > best_p:
            ...
            torch.save(save_data, "../save/downstream/best_sta_ori.pt")
            best_p = val_p
            # best_loss = val_loss
    ...
    # )
    # model_path = "../save/downstream/best_sta_ori.pt"
    # state_dict = torch.load(model_path)
    # state_sta = state_dict['model_state_dict']
    # model = model.model_down_tape.ProteinBertForValuePrediction()
    # model.load_state_dict({k.replace("module.", ""): v for k, v in state_sta.items()})
    ...
    #     stability_inputs, stability_targets = stability_inputs.cuda(), stability_targets.cuda()
    #     with torch.no_grad():
    #         outputs = model(stability_inputs, targets=stability_targets)
    #         loss, value_prediction = outputs
    #         p = spearmanr(stability_targets.detach().cpu().numpy(), value_prediction.detach().cpu().numpy())
    #
    #     test_loss += loss.item()
