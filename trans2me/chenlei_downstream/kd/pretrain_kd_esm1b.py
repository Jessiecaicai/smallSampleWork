...
if __name__ == '__main__':
    ...
    esm1b_pretrained_model, esm1b_alphabet = esm.pretrained.load_model_and_alphabet("../pretrained_models/esm1b_t33_650M_UR50S.pt")

    esm1b_pretrained_model = esm1b_pretrained_model.cuda()

    args = param_parser_esm1b.params_parser()

    esm1b_model = esm.model.ProteinBertModel(args, esm1b_alphabet)

    # model_path = "../save/kd/best_kd_esm1b_val.pt"
    ...
    esm1b_pretrained_model = convert_syncbn_model(esm1b_pretrained_model)
    esm1b_pretrained_model = amp.initialize(esm1b_pretrained_model, opt_level='O0')

    esm1b_model = convert_syncbn_model(esm1b_model)
    esm1b_model, optimizer = amp.initialize(esm1b_model, optimizer, opt_level='O0')

    esm1b_pretrained_model.eval()
    esm1b_model.train()
    best_loss = 100000
    best_ppl = 100000
    mlm_loss_weight = 0.5
    ...
    for epoch in range(epochs):
        ...
        for idx, batch in enumerate(msa_train_loader):
            ...
            with torch.no_grad():
                esm1b_pretrained_result = esm1b_pretrained_model(esm_mask_tokens)
                esm1b_pretrained_logits = esm1b_pretrained_result['logits']

            KD_loss = kl_loss(esm1b_logits, esm1b_pretrained_logits)
            ...
            if train_step > 0 and train_step % 10000 == 0:
                save_data = {"model_state_dict": esm1b_model.state_dict(),
                             ...
                             "epoch": epoch}
                print("Save model! ")
                torch.save(save_data, "../save/kd/best_kd_esm1b_process.pt")
        train_toc = time.time()
        ...
        for idx, batch in enumerate(msa_valid_loader):
            ...
            msa_mask_tokens, lm_labels = msa_mask_tokens.cuda(), lm_labels.cuda()
            esm_mask_tokens, esm_lm_labels = msa_mask_tokens.squeeze(0), lm_labels.squeeze(0)
            # esm_mask_tokens, esm_lm_labels = msa_mask_tokens.view(bs * num_seq, -1), lm_labels.squeeze(0).view(
            #     bs * num_seq, -1)
            with torch.no_grad():
                esm1b_pretrained_result = esm1b_pretrained_model(esm_mask_tokens)
                esm1b_pretrained_logits = esm1b_pretrained_result['logits']
        ...
        if val_ppl < best_ppl:
            ...
            torch.save(save_data, "../save/kd/best_kd_esm1b_val.pt")
            best_ppl = val_ppl
            # best_loss = val_loss
